...
class Adminpagetest(unittest.TestCase,AdminPage):
    ...
    def test1_adminmenu(self):
        driver=self.driver
        self.assertEquals(driver.current_url,"https://opensource-demo.orangehrmlive.com/index.php/dashboard", 'Invalid title')
    ...
    def test3_Qulification_adds(self):
        ...
        time.sleep(2)
        items = qulification.find_elements_by_tag_name("ul")
        print('ITEMS',items)
        for item in items:
            item.click()
            text = item.text
    ...
    def test4_admin_submenu_navigations(self):
